...
export function SessionsCalendarCard({ selected, sessions, onSelect }: SessionsCalendarCardProps) {
  // ponytail: mobile-only accordion; lg always shows via CSS
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="border-b border-border/60 bg-muted/20 p-0">
        <button
          type="button"
          className="flex w-full items-start justify-between gap-3 px-6 py-4 text-left lg:pointer-events-none"
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          aria-controls="sessions-schedule-calendar"
        >
          ...
          <ChevronDown
            className={cn(
              "mt-0.5 h-5 w-5 shrink-0 text-muted-foreground transition-transform lg:hidden",
              open && "rotate-180"
            )}
          />
        </button>
      ...
      <div
        id="sessions-schedule-calendar"
        className={cn(!open && "hidden", "lg:block")}
      >
